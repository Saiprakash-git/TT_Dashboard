...
export const generateTimetable = async (req, res) => {
  let semester = parseInt(req.query.semester || req.body.semester);
  if (isNaN(semester)) {
    const latest = await Semester.findOne().sort({ number: -1 }).lean().catch(() => null);
    if (latest) semester = latest.number;
  }
  if (isNaN(semester)) return res.status(400).json({ message: "semester required and must be a number" });
